# Google OAuth with labelhost

Google OAuth rejects `.localhost` subdomains as redirect URIs. This example shows how to use labelhost with a custom TLD to get a domain that Google accepts.
...
Any TLD in the Public Suffix List works. labelhost lets you set a custom TLD with `--tld`:

```bash
labelhost proxy start --tld dev
labelhost oauth-test next dev
# -> https://oauth-test.dev
```

`.dev` is a real gTLD (owned by Google). It requires HTTPS because it's HSTS-preloaded, which labelhost handles automatically.
...
```bash
labelhost proxy start --tld dev
labelhost oauth-test.local.ctate next dev
...
Labelhost defaults to HTTPS on port 443 (auto-elevates with sudo), so URLs don't need a port number.
...
The downside is you lose the labelhost benefits (named URLs, no port conflicts) for the OAuth callback flow.
