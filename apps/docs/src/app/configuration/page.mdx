...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>`hostnameTemplate`</td>
      <td>string</td>
      <td></td>
      <td>
        Hostname pattern using `{{ name }}` and `{{ worktree }}`.
      </td>
    </tr>
    <tr>
      <td>`script`</td>
...
For turborepo projects, use `labelhost` as the `dev` script and the real command in a separate script:
...
`pnpm dev` at the root runs turbo, which runs `labelhost` in each package. Labelhost detects the package manager and runs `pnpm run dev:app` through the proxy. No `turbo.json` changes are needed.

People without labelhost installed can run `pnpm run dev:app` directly.

### Precedence

Closest config wins, like Prettier and ESLint:

For `name`: CLI `--name` flag > `package.json` `"labelhost"` key > `labelhost.json` app entry > `package.json` inference.

For `script`: CLI `--script` flag > `package.json` `"labelhost"` key > `labelhost.json` app entry > default `"dev"`.

For `appPort`: CLI `--app-port` flag > `LABELHOST_APP_PORT` env var > `package.json` `"labelhost"` key > `labelhost.json` app entry > auto-assigned.
...
<table>
  ...
  <tbody>
    <tr>
      <td>`LABELHOST_PORT`</td>
      <td>Proxy port</td>
      <td>`443` (HTTPS) / `80` (HTTP)</td>
    </tr>
    <tr>
      <td>`LABELHOST_HTTPS`</td>
...
Labelhost stores state (routes, PID file, port file, TLS marker) in `~/.labelhost` on all platforms. Override with `LABELHOST_STATE_DIR`.
