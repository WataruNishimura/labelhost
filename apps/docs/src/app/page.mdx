...
Labelhost replaces port numbers with stable, named `.localhost` URLs for local development. For humans and agents.

```diff
- "dev": "next dev"                  # http://localhost:3000
+ "dev": "labelhost myapp next dev"   # https://myapp.localhost
```
...
Just run `labelhost`. It reads the `"dev"` script from `package.json` and runs it through the proxy:
...
The app name is inferred from `package.json`, git root, or directory name. Use a `labelhost.json` to override (e.g. `{ "name": "myapp" }` for `https://myapp.localhost`).
...
You can also use labelhost directly in scripts:

```json
{
  "scripts": {
    "dev": "labelhost myapp next dev"
  }
}
```
...
When multiple TLDs are configured, `LABELHOST_URL` uses the first TLD. `LABELHOST_TLD` accepts the same comma separated list format, e.g. `LABELHOST_TLD=localhost,test`.
...
Labelhost runs an HTTPS reverse proxy on port 443 by default. Each app registers a route mapping its hostname to an assigned port. Requests to `https://<name>.localhost` are proxied to the app.
