---
name: oauth
description: Configure OAuth providers (Google, Apple, Microsoft, Facebook, GitHub, etc.) to work with labelhost local dev URLs. Use when setting up OAuth redirect URIs, fixing "redirect_uri_mismatch" or "invalid redirect" errors, configuring sign-in providers for local development, or when a provider rejects .localhost subdomains. Triggers include "OAuth not working with labelhost", "redirect URI mismatch", "Google/Apple/Microsoft sign-in fails locally", "configure OAuth for local dev", or any task involving OAuth callback URLs with labelhost domains.
---

# OAuth with Labelhost

OAuth providers validate redirect URIs against domain rules. `.localhost` subdomains fail on most providers because they are not in the Public Suffix List or are explicitly blocked. Labelhost fixes this with `--tld` to serve apps on real, valid domains.

## The Problem

When labelhost uses the default `.localhost` TLD, OAuth providers reject redirect URIs like `http://myapp.localhost:1355/callback`:
...
```bash
labelhost proxy start --tld local.yourcompany.dev
labelhost myapp next dev
...
3. Add the labelhost domain to **Authorized JavaScript origins**: `https://myapp.dev`
...
HTTPS is required for `.dev` and `.app` (HSTS-preloaded). Labelhost handles this automatically with `--https`.
...
3. Configure Sign In with Apple, adding the labelhost domain as a **Return URL**: `https://myapp.dev/api/auth/callback/apple`

The domain must be a real, publicly-resolvable domain name. Since labelhost maps the domain to 127.0.0.1 locally, the browser resolves it but Apple's server-side validation may require the domain to resolve publicly too. If Apple rejects the domain, add a public DNS A record pointing to 127.0.0.1 for your dev subdomain.
...
Microsoft allows `http://localhost` with any port for development. It also accepts `.localhost` subdomains in most cases. Using a custom TLD with labelhost is still recommended for consistency across providers.

### Facebook (Meta)

1. Go to [Meta for Developers > App Dashboard](https://developers.facebook.com/apps/)
2. Under **Facebook Login > Settings**, add the labelhost URL to **Valid OAuth Redirect URIs**: `https://myapp.dev/api/auth/callback/facebook`
...
Set `NEXTAUTH_URL` to match the labelhost domain:
...
Set the `callbackURL` in each strategy to use the labelhost domain:
...
Read the `LABELHOST_URL` environment variable that labelhost injects into the child process:

```js
const baseUrl = process.env.LABELHOST_URL || "http://localhost:3000";
...
1. The provider's registered redirect URI matches the labelhost domain exactly (protocol, host, path)
2. `NEXTAUTH_URL` or equivalent is set to the labelhost URL (not `localhost`)
3. The proxy is running with the correct TLD (`labelhost list` to verify)
...
Labelhost defaults to HTTPS on port 443 (auto-elevates with sudo). Run `labelhost trust` to add the local CA to your system trust store and eliminate browser warnings.
...
The auth library is constructing the callback URL from `localhost` instead of the labelhost domain. Set the appropriate environment variable:

- **NextAuth**: `NEXTAUTH_URL=https://myapp.dev`
- **Auth.js v5**: `AUTH_URL=https://myapp.dev`
- **Manual**: `LABELHOST_URL` is injected automatically; use it as the base URL
