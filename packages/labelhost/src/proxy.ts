...
/** Response header used to identify a labelhost proxy (for health checks). */
export const LABELHOST_HEADER = "X-Labelhost";
 ...
 * labelhost proxy. Used to detect forwarding loops (e.g. a frontend dev
 * server proxying back through labelhost without rewriting the Host header).
 */
const LABELHOST_HOPS_HEADER = "x-labelhost-hops";

/**
 * Maximum number of times a request may pass through the labelhost proxy
 * before it is rejected as a loop. Two hops is normal when a frontend
 * proxies API calls to a separate labelhost-managed backend; five gives
...
export function createProxyServer(options: ProxyServerOptions): ProxyServer {
  ...
  const handleRequest = (req: http.IncomingMessage, res: http.ServerResponse) => {
    ...
    res.setHeader(LABELHOST_HEADER, "1");
    ...
    const hops = parseInt(req.headers[LABELHOST_HOPS_HEADER] as string, 10) || 0;
    if (hops >= MAX_PROXY_HOPS) {
      onError(
        `Loop detected for ${host}: request has passed through labelhost ${hops} times. ` +
          `This usually means a backend is proxying back through labelhost without rewriting ` +
      ...
      res.end(
        renderPage(
          508,
          "Loop Detected",
          `<div class="content"><p class="desc">This request has passed through labelhost ${hops} times. This usually means a dev server (Vite, webpack, etc.) is proxying requests back through labelhost without rewriting the Host header.</p><div class="section"><p class="label">Fix: add changeOrigin to your proxy config</p><pre class="terminal">proxy: {
  "/api": {
    ...
    if (!route) {
      ...
      res.end(
        renderPage(
          404,
          "Not Found",
          `<div class="content"><p class="desc">No app registered for <strong>${safeHost}</strong></p>${routesList}<div class="section"><div class="terminal"><span class="prompt">$ </span>labelhost ${safeSuggestion} your-command</div></div></div>`
        )
      );
      return;
    }
    ...
    proxyReqHeaders[LABELHOST_HOPS_HEADER] = String(hops + 1);
  ...
  const handleUpgrade = (req: http.IncomingMessage, socket: net.Socket, head: Buffer) => {
    ...
    if (hops >= MAX_PROXY_HOPS) {
      ...
      onError(
        `WebSocket loop detected for ${host}: request has passed through labelhost ${hops} times. ` +
      ...
      socket.end(
        "HTTP/1.1 508 Loop Detected\r\n" +
          ...
          "\r\n" +
          "Loop Detected: request has passed through labelhost too many times.\n" +
  ...
  const handleExtendedConnect = (req: http2.Http2ServerRequest, res: http2.Http2ServerResponse) => {
    ...
    // Classic CONNECT tunneling is not a labelhost feature; only WebSocket
  ...
  if (tls) {
    ...
    const plainServer = http.createServer((req, res) => {
      ...
      res.writeHead(302, { Location: location, [LABELHOST_HEADER]: "1" });
      res.end();
    });
