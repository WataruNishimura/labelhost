...
describe("loadConfig", () => {
  ...
  it("returns null when no labelhost.json exists", () => {
    expect(loadConfig(tmpDir)).toBeNull();
  });

  it("loads labelhost.json from cwd", () => {
  ...
  it("prefers labelhost.json over portless.json", () => {
    fs.writeFileSync(path.join(tmpDir, "labelhost.json"), JSON.stringify({ name: "mine" }));
    fs.writeFileSync(path.join(tmpDir, "portless.json"), JSON.stringify({ name: "upstream" }));
    expect(loadConfig(tmpDir)!.config.name).toBe("mine");
  });

  it("still validates portless.json when it is the one being used", () => {
  ...
  it('reads a package.json "labelhost" key', () => {
    fs.writeFileSync(
      path.join(tmpDir, "package.json"),
      JSON.stringify({ name: "pkg", labelhost: { name: "myapp" } })
    );
    expect(loadConfig(tmpDir)!.config.name).toBe("myapp");
  });

  it('falls back to a package.json "portless" key', () => {
    fs.writeFileSync(
      path.join(tmpDir, "package.json"),
      JSON.stringify({ name: "pkg", portless: { name: "myapp" } })
    );
    expect(loadConfig(tmpDir)!.config.name).toBe("myapp");
  });

  it('prefers the "labelhost" key over the "portless" key', () => {
    fs.writeFileSync(
      path.join(tmpDir, "package.json"),
      JSON.stringify({ name: "pkg", labelhost: { name: "mine" }, portless: { name: "upstream" } })
    );
    expect(loadConfig(tmpDir)!.config.name).toBe("mine");
  });
  ...
  it("loads config with all fields", () => {
    ...
    fs.writeFileSync(path.join(tmpDir, "labelhost.json"), JSON.stringify(config));
    const result = loadConfig(tmpDir);
  ...
  it("loads empty object config", () => {
    fs.writeFileSync(path.join(tmpDir, "labelhost.json"), "{}");
    const result = loadConfig(tmpDir);
    expect(result).not.toBeNull();
    expect(result!.config).toEqual({});
  });

  it("loads config from package.json labelhost key", () => {
    fs.writeFileSync(
      path.join(tmpDir, "package.json"),
      JSON.stringify({ name: "test", labelhost: { name: "myapp", script: "dev" } })
    );
    const result = loadConfig(tmpDir);
    expect(result).not.toBeNull();
    expect(result!.config.name).toBe("myapp");
  ...
  it("loads string shorthand from package.json labelhost key", () => {
    fs.writeFileSync(
      path.join(tmpDir, "package.json"),
      JSON.stringify({ name: "test", labelhost: "myapp" })
    );
    const result = loadConfig(tmpDir);
    expect(result).not.toBeNull();
    expect(result!.config.name).toBe("myapp");
  ...
  it("ignores empty string labelhost key", () => {
    fs.writeFileSync(
      path.join(tmpDir, "package.json"),
      JSON.stringify({ name: "test", labelhost: "" })
    );
    expect(loadConfig(tmpDir)).toBeNull();
  });

  it("prefers labelhost.json over package.json labelhost key", () => {
    fs.writeFileSync(path.join(tmpDir, "labelhost.json"), JSON.stringify({ name: "from-file" }));
    fs.writeFileSync(
      path.join(tmpDir, "package.json"),
      JSON.stringify({ name: "test", labelhost: { name: "from-pkg" } })
    );
    const result = loadConfig(tmpDir);
    expect(result!.config.name).toBe("from-file");
  });

  it("ignores package.json without labelhost key", () => {
    fs.writeFileSync(
      path.join(tmpDir, "package.json"),
  ...
  it("does not walk up to find package.json labelhost key", () => {
    fs.writeFileSync(
      path.join(tmpDir, "package.json"),
      JSON.stringify({ name: "test", labelhost: { name: "root-app" } })
    );
...
describe("loadConfig validation", () => {
  ...
  it("throws on invalid JSON", () => {
    fs.writeFileSync(path.join(tmpDir, "labelhost.json"), "not json");
    expect(() => loadConfig(tmpDir)).toThrow(ConfigValidationError);
  });
  ...
  it("throws when name is a number", () => {
    fs.writeFileSync(path.join(tmpDir, "labelhost.json"), JSON.stringify({ name: 42 }));
    expect(() => loadConfig(tmpDir)).toThrow(ConfigValidationError);
  });

  it("throws when name is empty string", () => {
    fs.writeFileSync(path.join(tmpDir, "labelhost.json"), JSON.stringify({ name: "" }));
    expect(() => loadConfig(tmpDir)).toThrow(ConfigValidationError);
  });
  ...
  it("throws when appPort is out of range", () => {
    fs.writeFileSync(path.join(tmpDir, "labelhost.json"), JSON.stringify({ appPort: 99999 }));
    expect(() => loadConfig(tmpDir)).toThrow(ConfigValidationError);
  });

  it("throws when appPort is a string", () => {
    fs.writeFileSync(path.join(tmpDir, "labelhost.json"), JSON.stringify({ appPort: "3000" }));
    expect(() => loadConfig(tmpDir)).toThrow(ConfigValidationError);
  });
  ...
  it("throws when proxy is not a boolean", () => {
    fs.writeFileSync(path.join(tmpDir, "labelhost.json"), JSON.stringify({ proxy: "false" }));
    expect(() => loadConfig(tmpDir)).toThrow(ConfigValidationError);
  });
  ...
  it("throws when turbo is not a boolean", () => {
    fs.writeFileSync(path.join(tmpDir, "labelhost.json"), JSON.stringify({ turbo: "yes" }));
    expect(() => loadConfig(tmpDir)).toThrow(ConfigValidationError);
  });
...
describe("loadPackageLabelhostConfig", () => {
  let tmpDir: string;
  ...
  it("returns config from package.json labelhost key", () => {
    fs.writeFileSync(
      path.join(tmpDir, "package.json"),
      JSON.stringify({ name: "test", labelhost: { name: "myapp", script: "start" } })
    );
    const result = loadPackageLabelhostConfig(tmpDir);
  ...
  it("returns null for empty string shorthand", () => {
    ...
    expect(loadPackageLabelhostConfig(tmpDir)).toBeNull();
  });

  it("returns null when no labelhost key", () => {
    fs.writeFileSync(path.join(tmpDir, "package.json"), JSON.stringify({ name: "test" }));
    expect(loadPackageLabelhostConfig(tmpDir)).toBeNull();
  });
  ...
  it("throws ConfigValidationError for invalid labelhost config", () => {
    fs.writeFileSync(
      path.join(tmpDir, "package.json"),
      JSON.stringify({ name: "test", labelhost: { appPort: "bad" } })
    );
    expect(() => loadPackageLabelhostConfig(tmpDir)).toThrow(ConfigValidationError);
  });
});
