...
export interface LabelhostConfig extends AppConfig {
  apps?: Record<string, AppConfig>;
  turbo?: boolean;
}

export interface LoadedConfig {
  config: LabelhostConfig;
  configDir: string;
}
  ...
  try {
    ...
    if (pkg && typeof pkg === "object") {
      for (const key of PACKAGE_KEYS) {
        ...
        const config = normalizeConfigValue(pkg[key]);
        if (config === null) return null;
        validateConfig(config, `${pkgPath} "${key}"`);
        return { config: config as LabelhostConfig, configDir: dir };
      }
    }
  } catch (err) {
...
/**
 * Load the `"labelhost"` config from a specific directory's package.json
 * (does not walk up). Returns the AppConfig fields or null.
 */
export function loadPackageLabelhostConfig(dir: string): AppConfig | null {
  const pkgPath = path.join(dir, "package.json");
  try {
    const raw = fs.readFileSync(pkgPath, "utf-8");
    const pkg = JSON.parse(raw);
 ...
 * command separators (newlines, operators) that splitCommand discards, so
 * callers can detect compound scripts the tokenizer would flatten.
 */
...
function validateConfig(config: unknown, configPath: string): asserts config is LabelhostConfig {
