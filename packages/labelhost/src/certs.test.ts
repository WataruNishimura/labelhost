...
describe("ensureCerts", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "labelhost-certs-test-"));
  });
  ...
  it("generates certs when state dir path contains a dot (#152)", () => {
    ...
    const dotDir = path.join(tmpDir, "user.name", ".labelhost");
...
describe("createSNICallback", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "labelhost-sni-test-"));
...
describe("isCATrusted", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "labelhost-trust-test-"));
  });
...
describe("trustCA", () => {
  ...
  it("returns error when CA cert is missing", () => {
    ...
    expect(result.error).toContain("labelhost trust");
  });
  ...
  it.skipIf(process.platform !== "darwin")(
    ...
    () => {
      ...
      const fakeBinDir = fs.mkdtempSync(path.join(os.tmpdir(), "labelhost-fake-sec-"));
...
describe("untrustCA", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "labelhost-untrust-test-"));
  });
  ...
  it("retries a failed Linux trust refresh after the source certificate is gone", () => {
    ...
    const installedCA = path.join(trustDir, "labelhost-ca.crt");
