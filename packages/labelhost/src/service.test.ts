...
describe("buildServiceSpec", () => {
  it("builds a macOS LaunchDaemon for the HTTPS proxy", () => {
    const spec = buildServiceSpec({
      ...
      entryScript: "/usr/local/lib/node_modules/labelhost/dist/cli.js",
    ...
    expect(spec.plistPath).toBe("/Library/LaunchDaemons/dev.labelhost.proxy.plist");
    expect(spec.programArguments).toEqual([
      "/usr/local/bin/node",
      "/usr/local/lib/node_modules/labelhost/dist/cli.js",
      "proxy",
      "start",
      "--foreground",
      "--port",
    ...
    expect(spec.plist).toContain("<key>LABELHOST_STATE_DIR</key>");
    expect(spec.plist).toContain("<string>/Users/alice/.labelhost</string>");
  ...
  it("builds a Linux systemd unit for the HTTPS proxy", () => {
    const spec = buildServiceSpec({
      ...
      entryScript: "/usr/lib/node_modules/labelhost/dist/cli.js",
      userHome: "/home/alice",
      uid: "1000",
      gid: "1000",
    });
    ...
    expect(spec.unitPath).toBe("/etc/systemd/system/labelhost.service");
    expect(spec.execStart).toEqual([
      "/usr/bin/node",
      "/usr/lib/node_modules/labelhost/dist/cli.js",
      "proxy",
      "start",
      "--foreground",
      "--port",
    ...
    expect(spec.unit).toContain("Description=Labelhost HTTPS proxy");
    expect(spec.unit).toContain('Environment=LABELHOST_STATE_DIR="/home/alice/.labelhost"');
    expect(spec.unit).toContain('Environment=SUDO_UID="1000"');
    expect(spec.unit).toContain(
      'ExecStart="/usr/bin/node" "/usr/lib/node_modules/labelhost/dist/cli.js" "proxy" "start" "--foreground" "--port" "443" "--https" "--skip-trust"'
    );
  ...
  it("builds a Windows startup task for the HTTPS proxy", () => {
    const spec = buildServiceSpec({
      ...
      entryScript: "C:\\Users\\Alice\\AppData\\Roaming\\npm\\node_modules\\labelhost\\dist\\cli.js",
      userHome: "C:\\Users\\Alice",
    });

    expect(spec.platform).toBe("win32");
    if (spec.platform !== "win32") throw new Error("Expected Windows service spec");
    expect(spec.taskName).toBe("Labelhost Proxy");
    expect(spec.scriptPath).toBe("C:\\ProgramData\\labelhost\\service\\labelhost-service.cmd");
    expect(spec.taskXmlPath).toBe("C:\\ProgramData\\labelhost\\service\\labelhost-task.xml");
    expect(spec.createArgs).toEqual([
      "/Create",
      "/TN",
      "Labelhost Proxy",
      "/XML",
      "C:\\ProgramData\\labelhost\\service\\labelhost-task.xml",
      "/F",
    ]);
    ...
    expect(spec.taskXml).toContain(
      "<Arguments>/d /s /c &quot;&quot;C:\\ProgramData\\labelhost\\service\\labelhost-service.cmd&quot;&quot;</Arguments>"
    );
    expect(spec.script).toContain("LABELHOST_STATE_DIR=C:\\Users\\Alice\\.labelhost");
  ...
  it("escapes percent signs in Windows batch env values", () => {
    ...
    if (spec.platform !== "win32") throw new Error("Expected Windows service spec");
    expect(spec.script).toContain("LABELHOST_STATE_DIR=C:\\Users\\100%%Done\\.labelhost");
  ...
  it("uses unconditional KeepAlive in the macOS plist", () => {
    const spec = buildServiceSpec({
      platform: "darwin",
      nodePath: "/usr/local/bin/node",
      entryScript: "/usr/local/lib/labelhost/cli.js",
      userHome: "/Users/bob",
      uid: "501",
      gid: "20",
    });
  ...
  it("persists LAN and wildcard settings in a macOS LaunchDaemon", () => {
    ...
    expect(spec.plist).toContain("<key>LABELHOST_LAN</key>");
    expect(spec.plist).toContain("<string>1</string>");
    expect(spec.plist).toContain("<key>LABELHOST_LAN_IP</key>");
    expect(spec.plist).toContain("<string>192.168.1.42</string>");
    expect(spec.plist).toContain("<key>LABELHOST_WILDCARD</key>");
  });
  ...
  it("persists no-TLS, custom TLD, and custom state in a Linux service", () => {
    ...
    expect(spec.unit).toContain('Environment=LABELHOST_STATE_DIR="/srv/labelhost"');
    expect(spec.unit).toContain('Environment=LABELHOST_PORT="8080"');
    expect(spec.unit).toContain('Environment=LABELHOST_HTTPS="0"');
    expect(spec.unit).toContain('Environment=LABELHOST_TLD="test"');
    expect(spec.stateDir).toBe("/srv/labelhost");
  });
});
...
describe("buildServiceUninstallSudoArgs", () => {
  it("builds a service-scoped sudo command that preserves user state", () => {
    const args = buildServiceUninstallSudoArgs("/fake/cli.js", {
      ...
      home: "/Users/alice",
      env: {
        LABELHOST_STATE_DIR: "/Users/alice/.labelhost",
        LABELHOST_DEBUG: "1",
        OTHER_ENV: "ignored",
      },
    });
...
describe("handleService", () => {
  ...
  it("prints help and exits 0 for --help", async () => {
    ...
    expect(output).toContain("labelhost service");
  ...
  it("registers the Windows task with no execution time limit before starting it", async () => {
    ...
    const taskXmlWrite = vi
      ...
      .mock.calls.find(
        ([file]) => file === "C:\\ProgramData\\labelhost\\service\\labelhost-task.xml"
      );
  ...
  it("keeps an atomically registered Windows task when immediate start fails", async () => {
    ...
    expect(logSpy.mock.calls.flat().join(" ")).toContain("Labelhost service installed.");
  });
  ...
  it("removes Windows service files even when no task was registered", async () => {
    ...
    expect(rmSync).toHaveBeenCalledWith("C:\\ProgramData\\labelhost\\service", {
      recursive: true,
      force: true,
    });
  });
  ...
  it("stops an existing proxy before restarting the Linux service", async () => {
    ...
    const restartIndex = calls.findIndex(
      (call) => call.command === "systemctl" && call.args.join(" ") === "restart labelhost.service"
    );
  ...
  it("stops the discovered proxy before a different install port", async () => {
    ...
    vi.mocked(discoverState).mockResolvedValue({
      dir: "/tmp/labelhost-service-current-test",
      port: currentPort,
      tls: true,
      tld: "localhost",
      tlds: ["localhost"],
  ...
  it("does not validate proxy env while uninstalling", async () => {
    setPlatform("linux");
    setGetuid(0);
    const originalPort = process.env.LABELHOST_PORT;
    const originalTld = process.env.LABELHOST_TLD;
    process.env.LABELHOST_PORT = "abc";
    process.env.LABELHOST_TLD = "bad-name";
    const runner = vi.fn((_: string, _args: string[]) => ({
      status: 0,
      stdout: "",
      stderr: "",
    ...
    try {
      ...
      if (originalPort === undefined) {
        delete process.env.LABELHOST_PORT;
      } else {
        process.env.LABELHOST_PORT = originalPort;
      }
      if (originalTld === undefined) {
        delete process.env.LABELHOST_TLD;
      } else {
        process.env.LABELHOST_TLD = originalTld;
      }
    }

    expect(exitSpy).not.toHaveBeenCalled();
    expect(runner).toHaveBeenCalledWith("systemctl", ["disable", "--now", "labelhost.service"]);
  });
  ...
  it("normalizes relative install paths before writing the Linux service", async () => {
    ...
    const unitWrite = vi
      .mocked(writeFileSync)
      .mock.calls.find(([file]) => file === "/etc/systemd/system/labelhost.service");
    const unit = String(unitWrite?.[1] ?? "");

    expect(mkdirSync).toHaveBeenCalledWith(stateDir, { recursive: true });
    expect(unit).toContain(`Environment=LABELHOST_STATE_DIR="${systemdStateDir}"`);
  ...
  it("prints installed service config from a Linux unit", async () => {
    ...
    expect(output).toContain("State directory: /srv/labelhost");
  });
});
