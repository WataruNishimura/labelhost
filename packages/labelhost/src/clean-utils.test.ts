...
describe("collectStateDirsForCleanup", () => {
  const prevState = process.env.LABELHOST_STATE_DIR;

  afterEach(() => {
    if (prevState === undefined) delete process.env.LABELHOST_STATE_DIR;
    else process.env.LABELHOST_STATE_DIR = prevState;
  });

  it("includes LABELHOST_STATE_DIR when the directory exists", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "labelhost-clean-collect-"));
    process.env.LABELHOST_STATE_DIR = tmp;
...
describe("removeLabelhostStateFiles", () => {
  let tmpDir: string;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "labelhost-clean-rm-"));
  });
  ...
  it("removes allowlisted files and host-certs directory", () => {
    ...
    removeLabelhostStateFiles(tmpDir);
  ...
  it("does not throw when paths are missing", () => {
    expect(() => removeLabelhostStateFiles(tmpDir)).not.toThrow();
  });
  ...
  it("retains the CA identity when trust removal must be retried", () => {
    ...
    removeLabelhostStateFiles(tmpDir, { preserveCAIdentity: true });
...
describe("attemptCATrustRemovalForCleanup", () => {
  it("attempts removal even when only one WSL trust store may contain the CA", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "labelhost-clean-trust-"));
    const untrust = vi.fn(() => ({
      removed: false,
