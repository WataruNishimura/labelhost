...
/** Filenames labelhost creates under a state directory (allowlisted for clean). */
const LABELHOST_STATE_FILES = [
  "routes.json",
  "routes.lock",
  "proxy.pid",
  "proxy.port",
...
export type RemoveLabelhostStateFilesOptions = {
 ...
 * dir, and LABELHOST_STATE_DIR when set.
 */
export function collectStateDirsForCleanup(): string[] {
  ...
  add(process.env.LABELHOST_STATE_DIR);
  return [...dirs];
}
...
/**
 * Best-effort removal of labelhost state files under dir. Only known filenames
 * are deleted; other files in the directory are left intact.
 */
export function removeLabelhostStateFiles(
  dir: string,
  options: RemoveLabelhostStateFilesOptions = {}
): void {
  for (const f of LABELHOST_STATE_FILES) {
