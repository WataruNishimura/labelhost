...
const SERVICE_LABEL = "dev.labelhost.proxy";
const SYSTEMD_SERVICE = "labelhost.service";
const WINDOWS_TASK_NAME = "Labelhost Proxy";
const INTERNAL_ELEVATED_ENV = "LABELHOST_INTERNAL_SERVICE_ELEVATED";
const SERVICE_ENV_KEYS = new Set(["LABELHOST_SYNC_HOSTS"]);
...
function parseServiceInstallConfig(
  ...
  if (env.LABELHOST_STATE_DIR) {
    config.stateDir = env.LABELHOST_STATE_DIR;
  }

  const envHttps = parseBooleanEnv(env.LABELHOST_HTTPS);
  if (envHttps !== null) {
    config.useHttps = envHttps;
  }

  const envLan = parseBooleanEnv(env.LABELHOST_LAN);
  if (envLan !== null) {
    config.lanMode = envLan;
  }

  if (env.LABELHOST_LAN_IP) {
    config.lanMode = true;
    config.lanIp = env.LABELHOST_LAN_IP;
    config.lanIpExplicit = true;
  }

  if (env.LABELHOST_TLD) {
    config.tlds = normalizeTlds(parseTldList(env.LABELHOST_TLD, "LABELHOST_TLD"));
    config.tld = primaryTld(config.tlds);
  }

  const envWildcard = parseBooleanEnv(env.LABELHOST_WILDCARD);
  if (envWildcard !== null) {
    config.useWildcard = envWildcard;
  }

  if (env.LABELHOST_PORT) {
    config.proxyPort = parsePortValue(env.LABELHOST_PORT, "LABELHOST_PORT");
  } else {
  ...
  if (!env.LABELHOST_PORT && !tokens.includes("--port") && !tokens.includes("-p")) {
    config.proxyPort = getProtocolPort(config.useHttps);
  }
...
function buildServiceEnv(ctx: ServiceContext): Record<string, string> {
  const env: Record<string, string> = {
    LABELHOST_STATE_DIR: ctx.stateDir,
    LABELHOST_PORT: ctx.config.proxyPort.toString(),
    LABELHOST_HTTPS: ctx.config.useHttps ? "1" : "0",
    LABELHOST_LAN: ctx.config.lanMode ? "1" : "0",
    LABELHOST_WILDCARD: ctx.config.useWildcard ? "1" : "0",
    ...ctx.config.extraEnv,
  };

  if (ctx.config.lanMode && ctx.config.lanIpExplicit && ctx.config.lanIp) {
    env.LABELHOST_LAN_IP = ctx.config.lanIp;
  }

  if (ctx.config.lanMode) {
    env.LABELHOST_TLD = "local";
  } else if (ctx.config.tlds.length > 1 || ctx.config.tld !== DEFAULT_TLD) {
    env.LABELHOST_TLD = ctx.config.tlds.join(",");
  }
...
function defaultStateDir(platform: SupportedPlatform, userHome: string): string {
  return platform === "win32"
    ? path.win32.join(userHome, ".labelhost")
    : path.posix.join(userHome, ".labelhost");
}
...
function buildSystemdUnit(ctx: ServiceContext, execStart: string[]): string {
  ...
  return `[Unit]
Description=Labelhost HTTPS proxy
...
<Task version="1.3" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    <Description>Labelhost HTTPS proxy</Description>
  </RegistrationInfo>
  <Triggers>
    <BootTrigger>
...
export function buildServiceSpec(options: {
  ...
  const scriptDir = path.win32.join(ctx.programData, "labelhost", "service");
  const scriptPath = path.win32.join(scriptDir, "labelhost-service.cmd");
  const taskXmlPath = path.win32.join(scriptDir, "labelhost-task.xml");
...
function currentServiceSpec(
  ...
  const stateDir =
    installConfig?.stateDir ||
    process.env.LABELHOST_STATE_DIR ||
...
function installedConfigFromSnapshot(
  ...
  try {
    ...
    const stateDir = parsed.stateDir || snapshot.env.LABELHOST_STATE_DIR || fallback.stateDir;
...
function collectLabelhostEnvArgs(
  env: NodeJS.ProcessEnv = process.env,
  omit: Set<string> = new Set()
): string[] {
  const envArgs: string[] = [];
  for (const key of Object.keys(env)) {
    if (key.startsWith("LABELHOST_") && env[key] && !omit.has(key)) {
...
function buildElevatedEnvArgs(options: {
  ...
  const overrideKeys = new Set(["LABELHOST_STATE_DIR", ...Object.keys(extraEnv)]);
  return [
    "env",
    ...collectLabelhostEnvArgs(options.env, overrideKeys),
    ...Object.entries(extraEnv).map(([key, value]) => `${key}=${value}`),
    `HOME=${options.home}`,
    `LABELHOST_STATE_DIR=${options.stateDir}`,
  ];
}
...
export function buildServiceUninstallSudoArgs(
  ...
  const stateDir = options.stateDir ?? env.LABELHOST_STATE_DIR ?? path.join(home, ".labelhost");
  return [
...
function requireUnixElevation(args: string[], runner: CommandRunner): void {
  ...
  const stateDir = process.env.LABELHOST_STATE_DIR || path.join(home, ".labelhost");
  const result = runner(
    "sudo",
    [
...
function prepareTrust(stateDir: string): void {
  ...
  console.log(colors.gray("Trusting labelhost CA for service startup..."));
  ...
  console.warn(colors.yellow("Run `labelhost trust` if browsers show certificate warnings."));
}
...
async function installService(
  ...
  console.log(colors.green("Labelhost service installed."));
...
async function uninstallService(entryScript: string, runner: CommandRunner): Promise<void> {
  ...
  console.log(colors.green("Labelhost service uninstalled."));
}

/**
 * Best-effort service removal for use by `labelhost clean`. Skips elevation
...
async function printServiceStatus(entryScript: string, runner: CommandRunner): Promise<void> {
  ...
  console.log(colors.bold("labelhost service"));
...
export function printServiceHelp(): void {
  console.log(`
${colors.bold("labelhost service")} - Start labelhost automatically when the OS starts.

${colors.bold("Usage:")}
  ${colors.cyan("labelhost service install")}             Install and start the HTTPS service on port 443
  ${colors.cyan("labelhost service install --lan")}       Enable LAN mode for the startup service
  ${colors.cyan("labelhost service install -p 8443")}     Use a custom proxy port
  ${colors.cyan("labelhost service uninstall")}           Stop and remove the startup service
  ${colors.cyan("labelhost service status")}              Show service and proxy status
...
${colors.bold("Notes:")}
  The service uses the default clean URL mode unless options or LABELHOST_*
