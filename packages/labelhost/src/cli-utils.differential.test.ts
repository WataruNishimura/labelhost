 ...
 *
 * The shell is the oracle. For each script shape below, run what labelhost
 * would actually run and read the child's argv:
 *
 *   VIOLATION  labelhost appended flags and the child never received them.
 *              This is the 502 class: exit 0, framework on its own port, and
 *              no test that asserts on the constructed array can see it.
 *   skip       labelhost declined. Conservative, documented, not a violation.
 *
 ...
 * Set LABELHOST_DIFFERENTIAL_RUNNERS=bun,npm,pnpm to widen the sweep past bun.
 */
...
/**
 * Every shape above that labelhost declines to inject into. Skipping is a
 ...
 */
const requestedRunners = process.env.LABELHOST_DIFFERENTIAL_RUNNERS?.split(",")
  .map((r) => r.trim())
  .filter(Boolean);
...
describe("package script forwarding, against the shell", () => {
  ...
  beforeAll(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), "labelhost-differential-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "labelhost-differential-fixture", scripts: SCRIPTS })
    );
  });
  ...
  function runScript(runner: string, name: string, extra: string[]) {
    ...
    // Mirror labelhost's own convention: npm needs `--` before script arguments.
  ...
  /**
   * Which framework runs and whether labelhost may append to the script are
  ...
  it("identifies the framework even in shapes it declines to touch", () => {
    ...
    // wrapper labelhost understands, must resolve regardless of skip status.
  ...
  for (const runner of runners) {
    it(`never appends flags the ${runner} child does not receive`, () => {
      ...
      expect(
        violations,
        `labelhost appended flags the shell discarded:\n  ${violations.join("\n  ")}`
      ).toEqual([]);
