...
function createTmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "labelhost-turbo-test-"));
}
  ...
  it("writes and reads manifest JSON", () => {
    const entries = {
      "/path/to/app": {
        PORT: "3001",
        HOST: "127.0.0.1",
        LABELHOST_URL: "https://app.project.local",
      },
    };
  ...
  it("includes NODE_EXTRA_CA_CERTS when provided", () => {
    const entries = {
      "/path/to/app": {
        ...
        HOST: "127.0.0.1",
        LABELHOST_URL: "https://app.project.local",
        NODE_EXTRA_CA_CERTS: "/home/user/.labelhost/ca.pem",
      },
    };
    writeManifest(entries, tmpDir);
    const raw = fs.readFileSync(manifestPath(tmpDir), "utf-8");
    expect(JSON.parse(raw)["/path/to/app"].NODE_EXTRA_CA_CERTS).toBe(
      "/home/user/.labelhost/ca.pem"
    );
  });
...
describe("loaderSource", () => {
  ...
  it("escapes backslashes for Windows paths", () => {
    const source = loaderSource("C:\\Users\\test\\.labelhost");
    expect(source).toContain("dev-manifest.json");
    ...
    expect(source).toContain("C:\\\\Users\\\\test\\\\.labelhost");
