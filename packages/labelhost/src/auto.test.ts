...
describe("inferProjectName", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "labelhost-test-"));
  });
...
describe("detectWorktreePrefix (git CLI path)", { timeout: 15_000 }, () => {
  ...
  try {
    ...
    const probe = fs.mkdtempSync(path.join(os.tmpdir(), "labelhost-git-probe-"));
    try {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "labelhost-git-test-"));
  });
