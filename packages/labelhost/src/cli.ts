...
function formatProxyStartCommand(proxyPort: number, config: ProxyConfig): string {
  ...
  return `${needsSudo ? "sudo " : ""}labelhost proxy start${args.length > 0 ? ` ${args.join(" ")}` : ""}`;
}
...
function printProxyConfigMismatch(
  ...
  console.error(chalk.cyan(`  ${needsSudo ? "sudo " : ""}labelhost proxy stop${portFlag}`));
  console.error(chalk.cyan(`  ${formatProxyStartCommand(proxyPort, desiredConfig)}`));
  process.exit(1);
}

/**
 * Return the path to the labelhost entry script. Guards against the
 * (unlikely) case where process.argv[1] is undefined.
 */
function getEntryScript(): string {
  const script = process.argv[1];
  if (!script) {
    throw new Error("Cannot determine labelhost entry script (process.argv[1] is undefined)");
  }
  return script;
}
 ...
 * nested run manages its own route instead of being proxied again. Accepts
 * the upstream `portless` name too, since a project may still use it.
 */
function isSelfCommand(command: string | undefined): boolean {
  return command === "labelhost" || command === "portless";
}
...
/**
 * Collect LABELHOST_* env vars as KEY=VALUE strings suitable for
 * `sudo env KEY=VAL ...` invocations (sudo may strip the environment).
 */
function collectLabelhostEnvArgs(): string[] {
  const envArgs: string[] = [];
  for (const key of Object.keys(process.env)) {
    if (key.startsWith("LABELHOST_") && process.env[key]) {
...
/**
 * Re-run `labelhost proxy stop` under sudo. Returns true if sudo succeeded.
 */
function sudoStop(port: number): boolean {
  ...
  const result = spawnSync("sudo", ["env", ...collectLabelhostEnvArgs(), ...stopArgs], {
    stdio: "inherit",
...
function startProxyServer(
  ...
  const lanIpPinned = !!process.env.LABELHOST_LAN_IP;
  ...
  const autoSyncHosts = shouldAutoSyncHosts(process.env.LABELHOST_SYNC_HOSTS);
  ...
  server.on("error", (err: NodeJS.ErrnoException) => {
    if (err.code === "EADDRINUSE") {
      ...
      console.error(colors.cyan("  labelhost proxy stop"));
      ...
      console.error(colors.cyan("  labelhost proxy start -p 1355"));
    } else {
...
function sudoStopOrHint(port: number): void {
  if (!isWindows) {
    if (!sudoStop(port)) {
      ...
      console.error(colors.cyan(`  labelhost proxy stop -p ${port}`));
    }
  } else {
    ...
    console.error(colors.cyan("  Run labelhost proxy stop as Administrator"));
  }
}
...
function listRoutes(store: RouteStore, proxyPort: number, tls: boolean): void {
  ...
  if (routes.length === 0) {
    ...
    console.log(colors.gray("Start an app with: labelhost <name> <command>"));
    return;
  }
...
function resolveProxyDesiredState(lanMode: boolean): ProxyDesiredState {
  ...
  const explicit: ProxyConfigExplicitness = {
    useHttps: process.env.LABELHOST_HTTPS !== undefined,
    customCert: false,
    lanMode: process.env.LABELHOST_LAN !== undefined,
    lanIp: process.env.LABELHOST_LAN_IP !== undefined,
    tlds: process.env.LABELHOST_TLD !== undefined,
    useWildcard: process.env.LABELHOST_WILDCARD !== undefined,
  };
  const desiredConfig = resolveProxyConfig({
    ...
    lanIp: process.env.LABELHOST_LAN_IP || null,
    tlds: envTlds,
...
async function ensureProxyRunning(
  ...
  const proxyListeningFromStateDir =
    !!process.env.LABELHOST_STATE_DIR && (await isPortListening(proxyPort));
...
async function runApp(
  ...
  const wantsFunnel = isEnabledEnv(process.env.LABELHOST_FUNNEL);
  const wantsTailscale = wantsFunnel || isEnabledEnv(process.env.LABELHOST_TAILSCALE);
  const wantsNgrok = isEnabledEnv(process.env.LABELHOST_NGROK);
  ...
  if (
    ...
    console.warn(
      chalk.yellow(
        `Warning: LABELHOST_TLD=${desired.envTlds.join(",")} but the running proxy uses ${formatTldList(tlds)}.`
      )
    );
  }
  ...
  // Ensure LABELHOST_LAN is propagated to child processes when the proxy
  // was started with --lan separately and discovered from the state marker,
  // not from the env var.
  if (lanMode && !process.env.LABELHOST_LAN) {
    process.env.LABELHOST_LAN = "1";
  }
  ...
  // Point Node.js at the labelhost CA so server-side fetches (e.g. Next.js
  // Server Components) trust labelhost-proxied HTTPS services. Node.js does
  // not use the system trust store, so without this env var it rejects the
  // labelhost CA as "self-signed certificate in certificate chain".
  ...
  console.log(
    chalk.gray(
      `Running: PORT=${port}${hostBind ? ` HOST=${hostBind}` : ""} LABELHOST_URL=${finalUrl}${caFragment} ${commandArgs.join(" ")}\n`
    )
  );
  ...
  spawnCommand(commandArgs, {
    env: {
      ...
      ...(lanMode ? { LABELHOST_LAN: "1" } : {}),
      ...(tailscaleUrl ? { LABELHOST_TAILSCALE_URL: tailscaleUrl } : {}),
      ...(ngrokUrl ? { LABELHOST_NGROK_URL: ngrokUrl } : {}),
      ...caEnv,
    },
    onCleanup: () => {
      stoppingNgrok = true;
...
function appPortFromEnv(): number | undefined {
  const envVal = process.env.LABELHOST_APP_PORT;
  if (!envVal) return undefined;
  const port = parseInt(envVal, 10);
  if (isNaN(port) || port < 1 || port > 65535) {
    console.error(colors.red(`Error: Invalid LABELHOST_APP_PORT="${envVal}". Must be 1-65535.`));
    process.exit(1);
  }
  return port;
}
...
function applySharingFlag(flag: string): boolean {
  if (flag === "--tailscale") {
    process.env.LABELHOST_TAILSCALE = "1";
    return true;
  }
  if (flag === "--funnel") {
    process.env.LABELHOST_FUNNEL = "1";
    process.env.LABELHOST_TAILSCALE = "1";
    return true;
  }
  if (flag === "--ngrok") {
    process.env.LABELHOST_NGROK = "1";
    return true;
  }
  return false;
}
...
function parseRunArgs(args: string[]): ParsedRunArgs {
  ...
  while (i < args.length && args[i].startsWith("-")) {
    if (args[i] === "--") {
      ...
      console.log(`
${colors.bold("labelhost run")} - Infer project name and run through the proxy.

${colors.bold("Usage:")}
  ${colors.cyan("labelhost run [options] [command...]")}
...
${colors.bold("Examples:")}
  labelhost run                        # Run dev script through proxy
  labelhost run next dev               # -> https://<project>.localhost
  labelhost run --name myapp next dev  # -> https://myapp.localhost
  labelhost run vite dev               # -> https://<project>.localhost
  labelhost run --app-port 3000 pnpm start
`);
      process.exit(0);
      ...
      if (!args[i] || args[i].startsWith("-")) {
        ...
        console.error(colors.cyan("  labelhost run --name <name> <command...>"));
        process.exit(1);
      }
      name = args[i];
...
function parseAppArgs(args: string[]): ParsedAppArgs {
  ...
  // Allow flags immediately after name (e.g. `labelhost myapp --force next dev`)
  while (i < args.length && args[i].startsWith("--")) {
    if (args[i] === "--") {
      i++;
      break;
...
function printHelp(): void {
  console.log(`
${colors.bold("labelhost")} - Replace port numbers with stable, named .localhost URLs. For humans and agents.
...
${colors.bold("Usage:")}
  ${colors.cyan("labelhost")}                         Run dev script through proxy
  ${colors.cyan("labelhost")}                         From monorepo root: run all workspace packages
  ${colors.cyan("labelhost run")}                     Same as above
  ${colors.cyan("labelhost run <cmd>")}               Run a command through the proxy
  ${colors.cyan("labelhost <name> <cmd>")}            Run with an explicit app name
  ${colors.cyan("labelhost proxy start")}             Start the proxy (HTTPS on port 443, daemon); rarely needed since it auto-starts on first run
  ${colors.cyan("labelhost proxy stop")}              Stop the proxy
  ${colors.cyan("labelhost service install")}         Start proxy automatically when the OS starts
  ${colors.cyan("labelhost get <name>")}              Print URL for a service (for cross-service refs)
  ${colors.cyan("labelhost alias <name> <port>")}     Register a static route (e.g. for Docker)
  ${colors.cyan("labelhost alias --remove <name>")}   Remove a static route
  ${colors.cyan("labelhost list")}                    Show active routes
  ${colors.cyan("labelhost doctor")}                  Check local labelhost health
  ${colors.cyan("labelhost trust")}                   Add local CA to system trust store
  ${colors.cyan("labelhost clean")}                   Remove labelhost state, trust entry, and hosts block
  ${colors.cyan("labelhost prune")}                   Kill orphaned dev servers from crashed sessions
  ${colors.cyan("labelhost hosts sync")}              Add routes to ${HOSTS_DISPLAY} (fixes Safari)
  ${colors.cyan("labelhost hosts clean")}             Remove labelhost entries from ${HOSTS_DISPLAY}

${colors.bold("Examples:")}
  labelhost                            # Run dev script through proxy
  labelhost                            # From monorepo root: start all apps
  labelhost --script start             # Run "start" script instead of "dev"
  labelhost myapp next dev             # -> https://myapp.localhost
  labelhost run next dev               # -> https://<project>.localhost
  labelhost run next dev               # in worktree -> https://<worktree>.<project>.localhost
  labelhost service install            # Start HTTPS proxy on OS startup
  labelhost service install --lan      # Persist LAN mode in the startup service
  labelhost service install --wildcard # Persist wildcard routing in the startup service
  labelhost get backend                # -> https://backend.localhost
  labelhost doctor                     # Check proxy, routes, DNS, and CA trust
  labelhost myapp --tailscale next dev # -> also https://<node>.ts.net (tailnet)
  labelhost myapp --funnel next dev    # -> also https://<node>.ts.net (public)
  labelhost myapp --ngrok next dev     # -> also https://<random>.ngrok.app (public)

${colors.bold("Configuration (labelhost.json):")}
  Optional. Labelhost works out of the box by running the "dev" script
  from package.json. Use labelhost.json to override defaults.
...
${colors.bold("How it works:")}
  ...
  5. Frameworks that ignore PORT (Vite, VitePlus, Astro, React Router, Angular,
     ...
     the like reject them, and an invocation labelhost cannot classify is left
     alone too. Expo --localhost, --lan and --tunnel modes are preserved while
     the assigned port is still injected.
     Labelhost also leaves a script alone when it is
  ...
  Elevated proxy processes keep the invoking user's ~/.labelhost state directory.
...
${colors.bold("HTTP/2 + HTTPS (default):")}
  ...
  On first use, labelhost generates a local CA and adds it to your
  system trust store. No browser warnings. Disable with --no-tls.
  On WSL, labelhost also adds the CA to the Windows user certificate store.
...
${colors.bold("LAN mode:")}
  ...
  Use LABELHOST_LAN=0 for one start to switch back to .localhost mode.
  If a proxy is already running with different explicit LAN/TLS/TLD settings,
  stop it first.
  ${colors.cyan("labelhost proxy start --lan")}
  ${colors.cyan("labelhost proxy start --lan --https")}
  ${colors.cyan("labelhost proxy start --lan --ip 192.168.1.42")}
...
${colors.bold("Tailscale sharing:")}
  ...
  on your tailnet.
  ${colors.cyan("labelhost myapp --tailscale next dev")}
  ${colors.cyan("labelhost myapp --funnel next dev")}
...
${colors.bold("ngrok sharing:")}
  ...
  ${colors.cyan("labelhost myapp --ngrok next dev")}
...
${colors.bold("Safari / DNS:")}
  ...
  custom TLDs, and LAN .local). Set LABELHOST_SYNC_HOSTS=0 to disable. To manually sync:
    ${colors.cyan("labelhost hosts sync")}
  Clean up later with:
...
${colors.bold("Reserved names:")}
  ...
  cannot be used as app names directly. Use "labelhost run" to infer the name,
  or "labelhost --name <name>" to force any name including reserved ones.
`);
  process.exit(0);
}
...
async function handleTrust(): Promise<void> {
  ...
  if (result.trusted) {
    ...
    console.log(colors.gray("Browsers will now trust labelhost HTTPS certificates."));
    return;
  }
...
async function handleClean(args: string[]): Promise<void> {
  if (args[1] === "--help" || args[1] === "-h") {
    console.log(`
${colors.bold("labelhost clean")} - Remove labelhost artifacts from this machine.

Stops the proxy if it is running, uninstalls the startup service if installed,
removes the local CA from the OS trust store when it was installed by labelhost,
deletes known files under state directories (~/.labelhost, the system state
directory, and LABELHOST_STATE_DIR when set), and removes the labelhost block
from ${HOSTS_DISPLAY}.
...
`);
  ...
  if (args.length > 1) {
    ...
    console.error(colors.cyan("  labelhost clean --help"));
    process.exit(1);
  }
      ...
      console.warn(
        colors.yellow(
          `Could not remove CA from trust store: ${untrustResult.error ?? "unknown error"}\n` +
            `Try: sudo labelhost clean (Linux), or delete the certificate manually.`
        )
      );
    }
  }
  ...
  for (const stateDir of stateDirs) {
    removeLabelhostStateFiles(stateDir, {
      preserveCAIdentity: failedCAStateDirs.has(stateDir),
    });
  }
  console.log(colors.green("Removed labelhost state files from known state directories."));
  ...
  if (cleanHostsFile()) {
    console.log(colors.green(`Removed labelhost entries from ${HOSTS_DISPLAY}.`));
  } else if (!isWindows && process.getuid?.() !== 0) {
    if (!runCleanWithSudo(`Updating ${HOSTS_DISPLAY} requires elevated privileges.`)) {
      console.error(colors.red(`Failed to update ${HOSTS_DISPLAY}. Run: sudo labelhost clean`));
      process.exit(1);
    }
  } else {
    console.warn(
      colors.yellow(
        `Could not remove labelhost entries from ${HOSTS_DISPLAY}${isWindows ? " (run as Administrator)." : "."}`
      )
    );
  }
...
async function handlePrune(args: string[]): Promise<void> {
  if (args[1] === "--help" || args[1] === "-h") {
    console.log(`
${colors.bold("labelhost prune")} - Kill orphaned dev servers left behind by crashed labelhost sessions.

When labelhost is killed with SIGKILL (kill -9) or crashes, child dev servers
...
${colors.bold("Usage:")}
  ${colors.cyan("labelhost prune")}
  ${colors.cyan("labelhost prune --force")}     Send SIGKILL instead of SIGTERM
...
async function handleGet(args: string[]): Promise<void> {
  if (args[1] === "--help" || args[1] === "-h") {
    console.log(`
${colors.bold("labelhost get")} - Print the URL for a service.

${colors.bold("Usage:")}
  ${colors.cyan("labelhost get <name>")}

Constructs the URL using the same hostname and worktree logic as
"labelhost run", then prints it to stdout. Useful for wiring services
together:

  BACKEND_URL=$(labelhost get backend)
...
${colors.bold("Examples:")}
  labelhost get backend                  # -> https://backend.localhost
  labelhost get backend                  # in worktree -> https://auth.backend.localhost
  labelhost get backend --no-worktree    # -> https://backend.localhost (skip worktree)
`);
    process.exit(0);
  }
  ...
  if (positional.length === 0) {
    ...
    console.error(colors.cyan("  labelhost get <name>"));
    console.error(colors.blue("Example:"));
    console.error(colors.cyan("  labelhost get backend"));
    process.exit(1);
  }
  ...
  const effectiveName = appConfig?.hostnameTemplate
    ...
    : applyWorktreePrefix(name, worktree);
  ...
  // Print bare URL to stdout so it works in $(labelhost get <name>)
...
async function handleAlias(args: string[]): Promise<void> {
  if (args[1] === "--help" || args[1] === "-h") {
    console.log(`
${colors.bold("labelhost alias")} - Register a static route for services not managed by labelhost.

${colors.bold("Usage:")}
  ${colors.cyan("labelhost alias <name> <port>")}        Register a route
  ${colors.cyan("labelhost alias --remove <name>")}      Remove a route
  ${colors.cyan("labelhost alias <name> <port> --force")} Override existing route

${colors.bold("Examples:")}
  labelhost alias my-postgres 5432     # -> https://my-postgres.localhost
  labelhost alias redis 6379           # -> https://redis.localhost
  labelhost alias --remove my-postgres # Remove the alias
`);
    process.exit(0);
  }
  ...
  if (args[1] === "--remove") {
    ...
    if (!aliasName) {
      ...
      console.error(colors.cyan("  labelhost alias --remove <name>"));
      process.exit(1);
    }
  ...
  if (!aliasName || !aliasPort) {
    ...
    console.error(colors.blue("Usage:"));
    console.error(colors.cyan("  labelhost alias <name> <port>"));
    console.error(colors.cyan("  labelhost alias --remove <name>"));
    console.error(colors.blue("Example:"));
    console.error(colors.cyan("  labelhost alias my-postgres 5432"));
    process.exit(1);
  }
...
async function handleHosts(args: string[]): Promise<void> {
  if (args[1] === "--help" || args[1] === "-h") {
    console.log(`
${colors.bold("labelhost hosts")} - Manage ${HOSTS_DISPLAY} entries for .localhost subdomains.
...
`);
  ...
  if (args[1] === "clean") {
    ...
    if (!isWindows && process.getuid?.() !== 0) {
      ...
      const result = spawnSync(
        "sudo",
        ["env", ...collectLabelhostEnvArgs(), process.execPath, getEntryScript(), "hosts", "clean"],
        {
          stdio: "inherit",
          timeout: SUDO_SPAWN_TIMEOUT_MS,
        }
  ...
  if (!args[1]) {
    console.log(`
${colors.bold("Usage: labelhost hosts <command>")}
...
`);
  ...
  if (args[1] !== "sync") {
    ...
    console.error(colors.blue("Usage:"));
    console.error(colors.cyan(`  labelhost hosts sync    # Add routes to ${HOSTS_DISPLAY}`));
    console.error(colors.cyan("  labelhost hosts clean   # Remove labelhost entries"));
    process.exit(1);
  }
  ...
  if (!isWindows && process.getuid?.() !== 0) {
    ...
    const result = spawnSync(
      "sudo",
      ["env", ...collectLabelhostEnvArgs(), process.execPath, getEntryScript(), "hosts", "sync"],
      {
        stdio: "inherit",
        timeout: SUDO_SPAWN_TIMEOUT_MS,
      }
...
function doctorProxyStartHint(proxyPort: number, tls: boolean): string {
  ...
  return `Run: labelhost proxy start${portArgs}${tlsArgs}`;
}

async function handleDoctor(args: string[]): Promise<void> {
  if (args[1] === "--help" || args[1] === "-h") {
    console.log(`
${colors.bold("labelhost doctor")} - Check local labelhost health and print suggested fixes.

${colors.bold("Usage:")}
  ${colors.cyan("labelhost doctor")}
...
stop, clean, prune, trust, or modify labelhost state.
...
`);
  ...
  if (args.length > 1) {
    console.error(colors.red(`Error: Unknown argument "${args[1]}".`));
    console.error(colors.cyan("  labelhost doctor --help"));
    process.exit(1);
  }
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    add("fail", `Could not discover labelhost state: ${message}`);
    state = {
      dir: resolveStateDir(),
  ...
  if (nodeMajor >= 24) {
    add("ok", `Node.js ${process.versions.node} satisfies labelhost requirements.`);
  } else {
  ...
  if (proxyRunning) {
    ...
    add(
      "fail",
      `Port ${proxyPort} is in use, but it is not a labelhost proxy.`,
  ...
  if (fs.existsSync(store.pidPath)) {
    try {
      ...
      if (isNaN(pid) || pid <= 0) {
        ...
        add("warn", `Proxy PID file is stale: ${pid}`, "Run: labelhost proxy stop");
      } else if (!proxyRunning) {
        add(
          "warn",
          `Proxy PID file points to PID ${pid}, but no labelhost proxy is responding on port ${proxyPort}.`,
          "Run: labelhost proxy stop"
        );
      } else {
  ...
  for (const { route, invalidPort, listening } of routePortChecks) {
    if (invalidPort) {
      add(
        ...
        route.pid === 0 ? "Remove or recreate the alias." : "Run: labelhost prune"
      );
      continue;
    }
...
async function handleProxy(args: string[]): Promise<void> {
  ...
  if (isProxyHelp || args[1] !== "start") {
    console.log(`
${colors.bold("labelhost proxy")} - Manage the labelhost proxy server.

${colors.bold("Usage:")}
  ${colors.cyan("labelhost proxy start")}                Start the HTTPS proxy on port 443 (daemon)
  ${colors.cyan("labelhost proxy start --no-tls")}       Start without HTTPS (port 80)
  ${colors.cyan("labelhost proxy start --lan")}          Enable LAN mode (mDNS, .local TLD)
  ${colors.cyan("labelhost proxy start --foreground")}   Start in foreground (for debugging)
  ${colors.cyan("labelhost proxy start -p 1355")}        Start on a custom port (no sudo)
  ${colors.cyan("labelhost proxy start --tld test")}     Use .test instead of .localhost
  ${colors.cyan("labelhost proxy start --tld localhost --tld test")}  Serve both TLDs
  ${colors.cyan("labelhost proxy start --tld dev.example.com")}  Use a multi-segment TLD (production parity)
  ${colors.cyan("labelhost proxy start --wildcard")}     Allow unregistered subdomains to fall back to parent
  ${colors.cyan("labelhost proxy stop")}                 Stop the proxy
...
`);
  ...
  // HTTPS is on by default. Disable with --no-tls or LABELHOST_HTTPS=0.
  ...
  if (portFlagIndex !== -1) {
    ...
    if (!portValue || portValue.startsWith("-")) {
      ...
      console.error(colors.blue("Usage:"));
      console.error(colors.cyan("  labelhost proxy start -p 8080"));
      process.exit(1);
    }
  ...
  const explicit: ProxyConfigExplicitness = {
    useHttps:
      ...
      customCertPath !== null ||
      customKeyPath !== null ||
      process.env.LABELHOST_HTTPS !== undefined,
    customCert: customCertPath !== null || customKeyPath !== null,
    lanMode: process.env.LABELHOST_LAN !== undefined,
    lanIp: process.env.LABELHOST_LAN_IP !== undefined,
    tlds: tldFlagValues.length > 0 || process.env.LABELHOST_TLD !== undefined,
    useWildcard: args.includes("--wildcard") || process.env.LABELHOST_WILDCARD !== undefined,
  };
  ...
  if (!hasExplicitPort) {
    ...
    if (
      ...
      (!!process.env.LABELHOST_STATE_DIR && (await isPortListening(currentState.port)))
    ) {
  ...
  const syncDisabled =
    process.env.LABELHOST_SYNC_HOSTS === "0" || process.env.LABELHOST_SYNC_HOSTS === "false";
  ...
  if (nonDefaultTlds.length > 0 && !lanMode && syncDisabled) {
    ...
    console.warn(colors.cyan("  labelhost hosts sync"));
  }
  ...
  if (proxyRunning) {
    ...
    console.log(
      colors.blue(`To restart: labelhost proxy stop${portFlag} && labelhost proxy start${portFlag}`)
    );
    return;
  }
  ...
  if (lanMode) {
    ...
    if (!lanIp) {
      ...
      console.error(colors.cyan("  labelhost proxy start --lan --ip 192.168.1.42"));
      process.exit(1);
    }
  } else {
    delete process.env[INTERNAL_LAN_IP_ENV];
  ...
  if (!isWindows && proxyPort < PRIVILEGED_PORT_THRESHOLD && (process.getuid?.() ?? -1) !== 0) {
    ...
    const result = spawnSync("sudo", ["env", ...collectLabelhostEnvArgs(), ...startArgs], {
      stdio: "inherit",
      timeout: SUDO_SPAWN_TIMEOUT_MS,
    });
    ...
    if (!hasExplicitPort) {
      ...
      console.error(colors.blue("Try again (labelhost will prompt for sudo):"));
    ...
    if (customCertPath && customKeyPath) {
      ...
      if (!skipTrust && !isCATrusted(stateDir)) {
        ...
        if (trustResult.trusted) {
          console.log(
            colors.green("CA added to system trust store. Browsers will trust labelhost certs.")
          );
        } else {
          ...
          console.warn(colors.cyan("  labelhost trust"));
        }
      }
  ...
  if (!(await waitForProxy(proxyPort, undefined, undefined, useHttps))) {
    ...
    console.error(colors.cyan("  labelhost proxy start --foreground"));
    if (fs.existsSync(logPath)) {
      console.error(colors.gray(`Logs: ${logPath}`));
    }
    process.exit(1);
...
/**
 * Load the effective AppConfig for the current directory from labelhost.json.
...
/**
 * Zero-arg dispatch: `labelhost` with no arguments.
 ...
 * Config (labelhost.json / package.json "labelhost" key) is loaded for overrides
 * but is not required.
 */
...
async function handleDefaultSingle(
  ...
  const effectiveName = appConfig?.hostnameTemplate
    ...
    : applyWorktreePrefix(baseName, worktree);
...
interface MultiAppEntry {
  ...
  /** Labelhost hostname (e.g. "web.json-render") */
  name: string;
...
async function spawnProxiedApp(
  ...
  const usesLabelhost = isSelfCommand(app.commandArgs[0]);
  ...
  if (usesLabelhost) {
    env = pkgEnv;
    displayUrl = "(managed by labelhost)";
  } else {
    store = new RouteStore(stateDir, {
...
async function handleDefaultMulti(
  ...
  for (const pkg of packages) {
    ...
    try {
      pkgConfig = loadPackageLabelhostConfig(pkg.dir);
    } catch (err) {
      if (err instanceof ConfigValidationError) {
        console.error(colors.red(`Error: ${err.message}`));
        process.exit(1);
    ...
    // Merge (closest wins): package.json "labelhost" > labelhost.json app entry > defaults
    ...
    name = appOverride.hostnameTemplate
      ? renderHostnameTemplate(appOverride.hostnameTemplate, {
          name,
          worktree: worktree?.prefix,
        })
      : applyWorktreePrefix(name, worktree);
...
async function runWithTurbo(
  ...
  for (const app of proxiedApps) {
    const usesLabelhost = isSelfCommand(app.commandArgs[0]);
    if (usesLabelhost) {
      appUrls.push({ label: app.label, url: "(managed by labelhost)" });
      continue;
    }
...
async function handleRunMode(args: string[], globalScript?: string): Promise<void> {
  ...
  if (parsed.commandArgs.length === 0) {
    ...
    console.error(colors.blue("Usage:"));
    console.error(colors.cyan("  labelhost run <command...>"));
    console.error(colors.blue("Example:"));
    console.error(colors.cyan("  labelhost run next dev"));
    process.exit(1);
  }
...
async function handleNamedMode(args: string[]): Promise<void> {
  ...
  if (parsed.commandArgs.length === 0) {
    console.error(colors.red("Error: No command provided."));
    console.error(colors.blue("Usage:"));
    console.error(colors.cyan("  labelhost <name> <command...>"));
    console.error(colors.blue("Example:"));
    console.error(colors.cyan("  labelhost myapp next dev"));
    process.exit(1);
  }
  ...
  const effectiveName = appConfig?.hostnameTemplate
    ...
    : safeName;
  parseHostname(effectiveName, DEFAULT_TLD);
...
async function main() {
  ...
  // labelhost is installed as a project dependency the env vars still fire,
  ...
  const advanceLabelhostFlag = (index: number, localValueFlags: Set<string>): number | null => {
  ...
  const globalFlagEnd = (): number => {
    ...
    while (index < limit) {
      const next = advanceLabelhostFlag(index, leadingValueFlags);
      if (next === null) break;
      index = next;
    }
    ...
    if (mode === "run") {
      ...
      while (index < limit) {
        const next = advanceLabelhostFlag(index, runValueFlags);
        if (next === null) break;
        index = next;
      }
      return index;
    ...
    while (index < limit) {
      const next = advanceLabelhostFlag(index, namedValueFlags);
      if (next === null) break;
      index = next;
    }
    return index;
  ...
  if (ipResult === false) {
    ...
    console.error(chalk.cyan("  labelhost --lan --ip 192.168.1.42 run <command>"));
    process.exit(1);
  } else if (typeof ipResult === "string") {
    process.env.LABELHOST_LAN_IP = ipResult;
    process.env.LABELHOST_LAN = "1";
  }
  ...
  if (scriptResult === false) {
    ...
    console.error(colors.cyan("  labelhost --script start"));
    process.exit(1);
  }
  ...
  if (args[0] === "--name") {
    ...
    if (!args[0]) {
      ...
      console.error(colors.cyan("  labelhost --name <name> <command...>"));
      process.exit(1);
    }
    const skipLabelhost =
  ...
  // as child-command tokens, not labelhost subcommands.
  if (!isRunCommand) {
  ...
  // Run app (either `labelhost run <cmd>` or `labelhost <name> <cmd>`)
  if (isRunCommand) {
