...
describe("Windows CA store", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "labelhost-windows-ca-test-"));
  ...
  it("adds a WSL certificate through the Windows certutil executable", () => {
    const run = vi.fn<WindowsCACommandRunner>((command, args) => {
      ...
      if (command === "wslpath" && args[0] === "-w") {
        return "\\\\wsl.localhost\\Ubuntu\\home\\alice\\.labelhost\\ca.pem\n";
      }
      return "";
    });
    ...
    expect(run).toHaveBeenCalledWith(
      ...
      ["-addstore", "-user", "Root", "\\\\wsl.localhost\\Ubuntu\\home\\alice\\.labelhost\\ca.pem"],
