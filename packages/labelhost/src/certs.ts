...
/** Common Name used for the labelhost local CA. */
const CA_COMMON_NAME = "labelhost Local CA";
...
/**
 * Check if the labelhost CA is already installed in the system trust store.
 */
...
function isCATrustedLinux(
  ...
  const systemCertPath = path.join(config.certDir, "labelhost-ca.crt");
...
/**
 * Add the labelhost CA to the system trust store.
 *
...
export function trustCA(stateDir: string): { trusted: boolean; error?: string } {
  ...
  if (!fileExists(caCertPath)) {
    return {
      trusted: false,
      error: "CA certificate not found. Run labelhost trust to generate it.",
    };
  }
  ...
  try {
    if (process.platform === "darwin") {
      ...
      const dest = path.join(config.certDir, "labelhost-ca.crt");
      ...
      const hint =
        process.platform === "darwin"
          ? "The macOS security command timed out. This can happen when the " +
            ...
            "(or run: sudo killall securityd) and then: labelhost trust"
          : "The trust command timed out. Try: labelhost trust";
    ...
    if (
      ...
      return {
        trusted: false,
        error: "Permission denied. Try: labelhost trust",
      };
    }
...
/**
 * Remove the labelhost CA from the system trust store (inverse of trustCA).
...
export function untrustCALinux(
  ...
  if (errors.length > 0 || isCATrustedLinux(stateDir, activeConfig)) {
    return {
      ...
      error:
        errors.join("; ") ||
        "CA still trusted (remove labelhost-ca.crt and run the distro CA update command, often with sudo)",
    };
  }
...
function untrustCAWSL(stateDir: string, caCertPath: string): { removed: boolean; error?: string } {
  ...
  return {
    removed: false,
    error: errors.join("; ") || "Could not remove the labelhost CA from every WSL trust store",
  };
}
