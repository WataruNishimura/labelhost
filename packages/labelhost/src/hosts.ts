...
const HOSTS_PATH = isWindows
  ...
  : "/etc/hosts";
const MARKER_START = "# labelhost-start";
const MARKER_END = "# labelhost-end";
...
/**
 * Extract the labelhost-managed block from /etc/hosts content.
...
/**
 * Remove the labelhost-managed block from /etc/hosts content and return
...
/**
 * Build a labelhost-managed block for the given hostnames.
 */
 ...
 * Disabled only when `LABELHOST_SYNC_HOSTS` is `0` or `false` (opt-out).
 */
 ...
 * Replaces any existing labelhost-managed block. Requires root access.
...
/**
 * Remove the labelhost-managed block from /etc/hosts.
 * Returns true on success, false on failure.
 */
...
/**
 * Return the current labelhost-managed hostnames from /etc/hosts.
 */
